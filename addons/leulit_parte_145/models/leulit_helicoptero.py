...
class leulit_helicoptero(models.Model):
    ...
    def _compute_last_parte_dates(self):
        for item in self:
    ...
    date_last_parte = fields.Date(compute='_compute_last_parte_dates', string='Fecha último parte', store=False, compute_sudo=True)
    llegada_last_parte = fields.Many2one('leulit.helipuerto', compute='_compute_last_parte_dates', string='Llegada último parte', store=False, compute_sudo=True)
