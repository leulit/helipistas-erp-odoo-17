...
# e.g. odoo-bin -c odoo.conf -d test_db --test-tags leulit_ai_integration
@common.tagged('post_install', '-at_install', 'leulit_ai_integration')
class TestAISearchIntegration(common.TransactionCase):
    ...
    odoo-bin -c odoo.conf -d test_db --test-tags leulit_ai_integration
    """
    ...
    def setUp(self):
        ...
        if not api_key_env:
            api_key_config = self.env['ir.config_parameter'].sudo().get_param('leulit_ai.openrouter_api_key')
            if api_key_config:
                _logger.info("Using API key from Odoo config for integration tests")
                # Set API key to Odoo config (make sure we have it there)
                self.env['ir.config_parameter'].sudo().set_param('leulit_ai.openrouter_api_key', api_key_config)
        else:
            _logger.info("Using API key from environment variable for integration tests")
            # Use the environment variable key
            self.env['ir.config_parameter'].sudo().set_param('leulit_ai.openrouter_api_key', api_key_env)
            
        # Get the final configured key
        self.api_key = self.env['ir.config_parameter'].sudo().get_param('leulit_ai.openrouter_api_key')
