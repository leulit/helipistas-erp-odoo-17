...
class HrExpenseSheet(models.Model):
    ...
    @api.depends('expense_line_ids.total_amount','account_move_ids.amount_total')
    def _compute_amount(self):
        for sheet in self:
            sheet.total_amount = sum(sheet.expense_line_ids.mapped('total_amount')) + sum(sheet.account_move_ids.mapped('amount_total'))
