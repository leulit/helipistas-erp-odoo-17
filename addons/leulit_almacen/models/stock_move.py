        ...
        for move in moves_to_assign:
            ...
            if move._should_bypass_reservation():
                ...
                if move.move_orig_ids:
                    _logger.error('Move %s has parents, looking for available move lines', move.display_name)
                ...
                if missing_reserved_quantity and move.product_id.tracking == 'serial' and (move.picking_type_id.use_create_lots or move.picking_type_id.use_existing_lots):
                    _logger.error('Move %s is tracked by serial number, creating move lines with serial numbers', move.display_name)
                    for i in range(0, int(missing_reserved_quantity)):
                        move_line_vals_list.append(move._prepare_move_line_vals(quantity=1))
                elif missing_reserved_quantity:
                    _logger.error('Move %s is not tracked by serial number, creating a move line with the remaining quantity', move.display_name)
                    ...
                    if to_update:
                        ...
                        move_line_vals_list.append(move._prepare_move_line_vals(quantity=0))
                assigned_moves_ids.add(move.id)
