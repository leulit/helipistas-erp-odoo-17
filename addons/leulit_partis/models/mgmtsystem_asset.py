...
class MgmtSystemAsset(models.Model):
    ...
    Mapeo conceptual:
    - mgmtsystem.hazard (OCA) se usa para representar ACTIVOS de información
    - Añade valoración C-I-D según MAGERIT para cumplimiento PART-IS
    - Ciclo de revisión según normativa AESA
    
    Nota: Este módulo extiende mgmtsystem.hazard desde dos perspectivas:
    - Este archivo (mgmtsystem_asset.py): campos de ACTIVO
    - mgmtsystem_risk.py: campos de ANÁLISIS DE RIESGOS
    Odoo fusionará automáticamente ambas extensiones en el mismo modelo.
    """

    _inherit = "mgmtsystem.hazard"
    _description = "SGSI Asset (PART-IS/AESA)"
