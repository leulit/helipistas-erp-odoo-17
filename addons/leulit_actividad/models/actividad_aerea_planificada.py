...
class ActividadAereaPlanificadaChainRequest():
    _name = "ActividadAereaPlanificadaChainRequest"
...
class ActividadAereaPreVueloHandler(actividad.AbstractHandler):
    def handle(self, request: Any) -> Any:
        _logger.error("-AA--> ActividadAereaPreVueloHandler")
        if not request.error:
            o_iaa = request.env['leulit.item_actividad_aerea']
            for item in o_iaa.search([('fecha','=',request.fecha),('partner','=',request.partner)],order="fecha ASC, inicio ASC"):
                nitems = o_iaa.search([('fecha','=',item.fecha),('fin', '<=', item.inicio),('partner','=',item.partner.id)], order="fecha ASC, inicio ASC", count=True)
...
class CoeficienteMayoracionHandler(actividad.AbstractHandler):
    def handle(self, request: Any) -> Any:
        if not request.error:
            ...
            o_iaa = request.env['leulit.item_actividad_aerea']
            for item in o_iaa.search([('fecha','=',request.fecha),('partner','=',request.partner)],order="fecha ASC, inicio ASC"):
                nitems = o_iaa.search([('modelo','=','calendar.event'),('fecha','=',item.fecha),('inicio', '>=', item.fin),('partner','=',item.partner.id)], order="fecha ASC, inicio ASC", count=True)
    ...
    def handle(self, request: Any) -> Any:
        ...
        if not request.error:
            ...
            datos = {
                ...
                'inicio' : inicioitems,
                'fin' : finitems,
                'tiempo_amplia' : tiempo_amplia,
                'airtime' : airtime,
                'tiempo_desc_parcial' : tiempo_desc_parcial,
                'max_duracion' : max_duracion + tiempo_amplia,
                'tiempo' : tiempoaa,
                'prevista':True,
            ...
            _logger.error("-CalcularActividadAerea--> datos = %r",datos)
            _logger.error("-CalcularActividadAerea--> itemAA = %r",itemAA)    
            if itemAA and itemAA.id:
                itemAA.sudo().write(datos)
            else:
                itemAA = o_aa.sudo().create(datos)
            _logger.error("-CalcularActividadAerea--> itemAA.id = %r",itemAA.id)    
