...
- **AI universal search** (`leulit_ai`): natural-language search bar in the top navbar. The LLM picks models/fields and the module runs the query through the user's own ORM (permissions respected, hard limit of 100 records). Engine `leulit_ai.search.engine` (`models/ai_search.py`), JSON endpoints under `/leulit_ai/*`, favourites and reports with visualization. Providers: OpenRouter (default) or Vertex AI (Gemini), configured via `leulit_ai.*` system parameters — see the module's `README.md`. It replaces the former chat-sidebar assistant (`/ai/chat` + `ai-service`/`helipistas-mcp`/`litellm-proxy` containers), whose `CLAUDE.md`/`ARCHITECTURE.md` no longer exist. Non-negotiable rule: a feature only belongs here if it improves the user's actual daily work — no AI for AI's sake. Odoo 17 **Community** only — never reference Enterprise-only models/modules (`sign`, `documents`, `web_gantt`, `web_studio`, ...).
