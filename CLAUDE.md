...
- `ir.rule` + `RBase` gotcha: virtually every functional role chains up through `implied_ids` to `RBase`, so any privileged group (e.g. `hr.group_hr_user`) also carries `RBase`. Odoo combines record rules as (AND of global, groupless rules) × (OR of rules scoped to groups the user actually has) — if you add a restrictive rule scoped to `leulit.RBase` (e.g. "see only your own record"), a privileged user who should bypass it gets silently restricted too, unless you also add an explicit unrestricted `domain_force="[(1,'=',1)]"` rule scoped to their privileged group. **This is not enough on its own for `hr.employee`**: it's used ERP-wide as a lookup/reference model (calendars, rosters, pilot/crew assignment, dropdowns), not just as a personal-profile record, so *any* row-level restriction scoped to `leulit.RBase` — even with the officer bypass in place — breaks those features for every non-HR user, since they legitimately need to read colleagues' records, not just their own. A same-day attempt at "self record only for RBase, unrestricted for `hr.group_hr_user`" (commit `e85e8f1a`, 2026-09-02) broke calendar/roster views for ordinary pilots within hours and was reverted; self-service access to one's own `hr.employee` record needs a narrower mechanism (a scoped field/view, not a blanket `ir.rule` on the model) if it's attempted again. A module's `ir.ui.menu` groups can be overridden by reusing its full stock external id (e.g. `hr.menu_hr_root`) to add a group without dropping the existing ones — see `addons/leulit_operaciones/security.xml` for that pattern.
...
- **AI universal search** (`leulit_ia`): natural-language search bar in the top navbar. The LLM picks models/fields and the module runs the query through the user's own ORM (permissions respected, hard limit of 100 records). Engine `leulit_ia.search.engine` (`models/ai_search.py`), JSON endpoints under `/leulit_ia/*`, favourites and reports with visualization. Providers: OpenRouter (default) or Vertex AI (Gemini), configured via `leulit_ia.*` system parameters — see the module's `README.md`. It replaces the former chat-sidebar assistant (`/ai/chat` + `ai-service`/`helipistas-mcp`/`litellm-proxy` containers), whose `CLAUDE.md`/`ARCHITECTURE.md` no longer exist. Non-negotiable rule: a feature only belongs here if it improves the user's actual daily work — no AI for AI's sake. Odoo 17 **Community** only — never reference Enterprise-only models/modules (`sign`, `documents`, `web_gantt`, `web_studio`, ...).
