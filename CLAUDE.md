...
- `ir.rule` + `RBase` gotcha: virtually every functional role chains up through `implied_ids` to `RBase`, so any privileged group (e.g. `hr.group_hr_user`) also carries `RBase`. Odoo combines record rules as (AND of global, groupless rules) × (OR of rules scoped to groups the user actually has) — if you add a restrictive rule scoped to `leulit.RBase` (e.g. "see only your own record"), a privileged user who should bypass it gets silently restricted too, unless you also add an explicit unrestricted `domain_force="[(1,'=',1)]"` rule scoped to their privileged group. Reference: `addons/leulit_operaciones/security.xml`, self-service access to `hr.employee` (own record for `RBase`, unrestricted for `hr.group_hr_user`); the same file also shows overriding a stock module's `ir.ui.menu` groups by reusing its full external id (e.g. `hr.menu_hr_root`) to add a group without dropping the existing ones.
